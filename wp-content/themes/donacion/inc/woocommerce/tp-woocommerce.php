...
if( !function_exists('donacion_content_single_details') ) {
    function donacion_content_single_details( ) {
        global $product;
        global $post;
        global $woocommerce;
    ...
    <div class="tp-shop-details__right-warp">
        ...
        <?php if(!empty($rating)) : ?>
            <div class="tp-shop-details__ratting d-flex">
                ...
                <span class="review-text">( <?php echo esc_html($ratingcount); ?> <?php echo esc_html__('customer ', 'donacion'); echo esc_html($ratingcount) <= 1 ? 'review' : 'reviews'; ?> )</span>
            </div>
            <?php else : ?>
            <div class="tp-shop-details__ratting">
                ...
                <span><i class="fal fa-star"></i></span>
                <span><i class="fal fa-star"></i></span>
                <span>( <?php echo esc_html($ratingcount); ?> <?php echo esc_html__('customer ', 'donacion'); echo esc_html($ratingcount) <= 1 ? 'review' : 'reviews'; ?> )</span>
            </div>
        <?php endif; ?> 
...
add_action( 'woocommerce_single_product_summary', 'donacion_content_single_details', 4 );
...
*************************************************/
function donacion_shipping_progress_bar() {
...
if(get_theme_mod( 'shipping_progress_bar_location_card_page',0) == '1'){
    add_action( 'woocommerce_before_cart_table',  'donacion_shipping_progress_bar' );
}

if(get_theme_mod( 'shipping_progress_bar_location_mini_cart',0) == '1'){
    add_action( 'woocommerce_before_mini_cart_contents', 'donacion_shipping_progress_bar' );
}

if(get_theme_mod( 'shipping_progress_bar_location_checkout',0) == '1'){
    add_action( 'woocommerce_checkout_before_customer_details', 'donacion_shipping_progress_bar' );
}
...
function donacion_sale_percentage(){
   global $product;
   $output = '';
...
if ( ! function_exists( 'donacion_header_add_to_cart_fragment' ) ) {
    function donacion_header_add_to_cart_fragment( $fragments ) {
        ob_start();
        ?>
...
add_filter( 'woocommerce_add_to_cart_fragments', 'donacion_header_add_to_cart_fragment' );


// product-content archive
if( !function_exists('donacion_content_product_grid') ) {
    function donacion_content_product_grid( ) {
    global $product;
    global $post;
    global $woocommerce;
    $rating = wc_get_rating_html($product->get_average_rating());
        ...
        <div class="product__thumb w-img p-relative fix">
            ...
            <div class="product__action d-flex flex-column flex-wrap">
                ...
                <div class="product-action-btn product-add-wishlist-btn p-relative">
                    ...
                    <span class="product-action-tooltip"><?php echo esc_html__('Add To Wishlist','donacion'); ?></span>
                </div>
                <?php endif; ?>
                ...
                <div class="product-action-btn p-relative">
                    ...
                    <span class="product-action-tooltip"><?php echo esc_html__('Quick view','donacion'); ?></span>
                </div>
                <?php endif; ?>
                ...
                <div class="product-action-btn p-relative">
                    ...
                    <span class="product-action-tooltip"> <?php echo esc_html__('Add To Compare','donacion'); ?></span>
                </div>
                <?php endif; ?>
...
add_action( 'woocommerce_before_shop_loop_item', 'donacion_content_product_grid', 10 );


// smart quickview
add_filter( 'woosq_button_html', 'donacion_woosq_button_html', 10, 2 );
function donacion_woosq_button_html( $output , $prodid ) {
    ...
    $price_class = $price ? NULL : 'price-empty';
        ...
        if ( $product ) {
         ...
         if( $product->is_type( 'simple' ) ){
            $btntext = esc_html__("Add to Cart",'donacion');
         } elseif( $product->is_type( 'variable' ) ){
            $btntext = esc_html__("Select Options",'donacion');
         } elseif( $product->is_type( 'external' ) ){
            $btntext = esc_html__("Buy Now",'donacion');
         } elseif( $product->is_type( 'grouped' ) ){
            $btntext = esc_html__("View Products",'donacion');
         }
         else{
            $btntext = "Add to Cart";
         } 
        ...
        $args = wp_parse_args(
            ...
            apply_filters(
                ...
                array(
                    ...
                    'before'      => '',
                    'after'       => '',
                    'home'        => _x( 'Home', 'breadcrumb', 'donacion' ),
                )
            )
        );
...
/*************************************************
## donacion Woo Search Form
*************************************************/ 

add_filter( 'get_product_search_form' , 'donacion_custom_product_searchform' );

function donacion_custom_product_searchform( $form ) {

	$form = '<div class="search-form p-relative">
                <form action="' . esc_url( home_url( '/shop'  ) ) . '" role="get_search_form()" method="get" id="searchform">
                <input type="search" value="' . get_search_query() . '" name="s" placeholder="'.esc_attr__('Search Product','donacion').'" autocomplete="off">
